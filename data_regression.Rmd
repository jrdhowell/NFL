...
```{r}
#load packages
load.libraries <- c('tidyverse', 'fastDummies', 'caret', 'janitor', 'randomForest', 'glmnet', 'rpart')
...
y_predicted <- predict(fit.lasso, s = best_lambda, newx = x)
...
```{r}
y_predicted <- predict(fit.dt, train, method = "anova")
